...
def process_file(filepath):
    if filepath.endswith("index.html") and filepath != "./index.html":
        ...
        if "family=Playfair+Display" not in content:
            # Note: the group(0) will match something like <script src="https://cdn.tailwindcss.com?plugins=..."></script>
            # We add \1 \n font_link
            content = re.sub(r'(<script[^>]*tailwindcss\.com[^>]*></script>)', r'\1\n    ' + font_link, content)
...
for root, dirs, files in os.walk('.'):
    for f in files:
        if f.endswith('index.html'):
            ...
            if os.path.abspath(path) != os.path.abspath('index.html') and 'knowledge' not in path:
                process_file(path)
